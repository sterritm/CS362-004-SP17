...
int adventurerEffect(int drawntreasure, struct gameState *state, int currentPlayer, int *temphand, int z) {
	...
	while (z - 1 > 0) {		//bug - changed comparator to > from >=
...
int councilRoomEffect(int currentPlayer, struct gameState *state, int handPos) {
	...
	{
		drawCard(i, state);		//bug - removed if statement preventing current player from drawing card
	}
...
int smithyEffect(int currentPlayer, struct gameState *state, int handPos) {
	int i;
	//+3 cards
	for (i = 1; i < 3; i++)		//bug - changed i to start at 1 instead of 0
	{
		drawCard(currentPlayer, state);
	}
...
int minionEffect(int currentPlayer, struct gameState *state, int handPos, int choice1, int choice2) {
	...
	else if (choice2)		//discard hand, redraw 4, other players with 5+ cards discard hand and draw 4
	{
		...
		for (i = 0; i < state->numPlayers; i++)
		{
			if (i != currentPlayer)
			{
				if (state->handCount[i] > 5)	//bug - changed number to 5 from 4
				{
					//discard hand
